/**
 * KODA ESP32-S3 Firmware - FINAL v2
 * ─────────────────────────────────────────────────────────────────────────────
...
void loop() {
    ...
    if (now - lastSweep >= LIDAR_SWEEP_MS) {
        ...
        for (int i = 0; i < 360; i++) {
            ...
            validCount++;
            if (scanDist[i] < minD) { minD = scanDist[i]; minAngle = i; }
        }
        ...
        if (bleConnected && stableCount > 0) {
            ...
            for (int i = 0; i < 360; i++) {
                ...
                chunk[chunkIdx++] = scanDist[i] & 0xFF;
                chunk[chunkIdx++] = (scanDist[i] >> 8) & 0xFF;
                if (chunkIdx >= 176) {
                    lidarChar->setValue(chunk, chunkIdx);
                    lidarChar->notify();
                    chunkIdx = 0;
                    delay(30); // increased from 10 to prevent BLE MTU drops
                }
            }
            if (chunkIdx > 0) {
                lidarChar->setValue(chunk, chunkIdx);
